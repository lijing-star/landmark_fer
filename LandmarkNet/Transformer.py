...
from utils_train import _get_activation_fn
...
class Transformer(nn.Module):
    def __init__(self, num_points, d_model=256, nhead=8, num_decoder_layer=6, dim_feedforward=1024,
        ...
        self.structure_encoding = nn.Parameter(torch.randn(1, num_points + 1, d_model))
        # landmark query
        # self.landmark_query = nn.Parameter(torch.randn(1, num_points , d_model))
        self.landmark_query = nn.Parameter(torch.randn(1, num_points + 1, d_model))

        LandmarkNet_Inherent_Layer = Inherent_Layer(d_model, nhead, dim_feedforward, dropout,
                                                    activation, normalize_before)
        decoder_norm = nn.LayerNorm(d_model)
        self.Transformer_block = Transformer_block(LandmarkNet_Inherent_Layer, num_decoder_layer, decoder_norm, return_intermediate=True)
    ...
    def forward(self, src):
        ...
        # append cls token
        cls_token = self.cls_token +structure_encoding[:1, :, :]
        cls_tokens = cls_token.expand(-1,src.shape[1], -1)
