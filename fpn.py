...
def LResNet50E_IR(opts,is_gray=False, num_mask=101):
    filter_list = [64, 64, 128, 256, 512, num_mask]
    layers = [3, 4, 14, 3]
    model = LResNet(BlockIR, layers, filter_list, is_gray)

    pretrained = torch.load(opts.pretrain_backbone_IR)
    net_dict = model.state_dict()
    pretrained_dic = {k: v for k, v in pretrained['state_dict'].items() 
                if (k in model.state_dict()
                )}
    net_dict.update(pretrained_dic)
